...

	{
		...
		authGroup.Use(middlewares.LimitIP("1000-H"))
		{
			...
			usersGroup := v1.Group("/users")
			{
				...
				usersGroup.PUT("", middlewares.AuthJWT("api"), uc.Update)
				usersGroup.PATCH("/email", middlewares.AuthJWT("api"), uc.UpdateEmail)
				usersGroup.PATCH("/phone", middlewares.AuthJWT("api"), uc.UpdatePhone)
				usersGroup.PATCH("/password", middlewares.AuthJWT("api"), uc.UpdatePassword)
				usersGroup.PATCH("/avatar", middlewares.AuthJWT("api"), uc.UpdateAvatar)
			}
			...
			cgcGroup := v1.Group("/categories")
			{
				...
				cgcGroup.DELETE("/:id", middlewares.AuthJWT("api"), cgc.Delete)
			}

			tpc := new(api.TopicsController)
			tpcGroup := v1.Group("/topics")
			{
				tpcGroup.GET("", tpc.Index)
				tpcGroup.POST("", middlewares.AuthJWT("api"), tpc.Store)
				tpcGroup.GET("/:id", tpc.Show)
				tpcGroup.PUT("/:id", middlewares.AuthJWT("api"), tpc.Update)
				tpcGroup.DELETE("/:id", middlewares.AuthJWT("api"), tpc.Delete)
			}

			lsc := new(api.LinksController)
